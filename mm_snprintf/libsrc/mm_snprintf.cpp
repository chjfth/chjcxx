	...
	TCHAR buf[], int bufsize)
{
	...
	for(;;)
	{
		...
		if(i<mmquan(rresult))
		{
			rresult[i] = _T('0') + (TCHAR)remainder; // (TCHAR) conversion to avoid VC2019 Lv4 warning
			if(radix==RdxHex && remainder>=10)
			{
				rresult[i] = (TCHAR)(remainder-10) + (TCHAR)(ishex_uppercase?_T('A'):_T('a'));
			}
		}
...
// #endif
		...
		* (or as-is conversion specifier for unknown conversions) */
		...
		if (justify_left) {          /* right blank padding to the field width */
			int nx = min_field_width - (str_arg_l+number_of_zeros_to_pad);
			if (nx > 0) {
				
				TCHAR cfill = _T(' ');
				_mm_fillchars_ct(ctipack, cfill, nx);
				
				if (str_l < str_max) {
					int avail = str_max-str_l; //Chj
					_mm_fillchars(strbuf+str_l, cfill, (nx>avail?avail:nx));
				}
