...
FIB_ret ggt_FlexiInfo(HWND hwndOwner, const TCHAR *pszInfo); // simplified version of above


FIB_ret ggt_vlFlexiInfobox(
	HWND hwndOwner, const FibInput_st *p_usr_opt, const TCHAR *szfmt, va_list args);

FIB_ret ggt_vaFlexiInfobox(
	HWND hwndOwner, const FibInput_st *p_usr_opt, const TCHAR *szfmt, ...);

FIB_ret ggt_vaFlexiInfo(HWND hwndOwner, const TCHAR *szfmt, ...);
...
#if defined(FlexiInfobox_IMPL) || (defined CHHI_ALL_IMPL && !defined CHHI_ALL_IMPL_HIDE_FlexiInfobox) // [IMPL]
...
fib_CalNewboxTextMax(HWND hdlg, FibDlgParams_st *pr, Size_st *pIdealDrawsize=NULL)
{
	...
	if(pIdealDrawsize) {
		pIdealDrawsize->cx = RECTcx(drawarea);
		pIdealDrawsize->cy = RECTcy(drawarea);
	}
	...
	Rect_st rcParent;
	if(pr->hwndRealOwner)
		GetWindowRect(pr->hwndRealOwner, (RECT*)&rcParent);
	else {
		POINT pt = {200, 200};
		GetCursorPos(&pt);
		SetRect((RECT*)&rcParent, pt.x, pt.y, pt.x, pt.y);
	}

	RECT rcBox0 = rcParent.StrechKeepCenter(newbox_width, newbox_height);

	RECT rcBox1 = mumo_PlaceRectInsideScreen(rcBox0, false, true);

	if( RECTcx(rcBox1)<newbox_width || RECTcy(rcBox1)<newbox_height )
...
{
	if(pr->isLastTextTimeOnTitle)
	{
		struct tm t;
		_localtime64_s(&t, (__time64_t*)&pr->uesecLastText);
		TCHAR sztitle[40];
		snTprintf(sztitle, _T("%04d-%02d-%02d %02d:%02d:%02d"),
	...
	bool isAdjustWindowNow, int idUserCmd=0, bool isTickOn=false)
{
	...
	pr->uesecLastText = _time64(nullptr);
	...
	Rect_st rcNewBox = fib_CalNewboxTextMax(hdlg, pr, &idealEditSize); // return Dlg rect, not Editbox's

	if( RECTcx(rcNewBox) > RECTcx(pr->rectNewboxVisualMax) )
	{
		pr->rectNewboxVisualMax.left = rcNewBox.left;
		pr->rectNewboxVisualMax.right = rcNewBox.right;
		isGoBigger = true;
	}

	if( RECTcy(rcNewBox) > RECTcy(pr->rectNewboxVisualMax) )
	{
		pr->rectNewboxVisualMax.top = rcNewBox.top;
		pr->rectNewboxVisualMax.bottom = rcNewBox.bottom;
		isGoBigger = true;
	}
	...
	if(isGoBigger && wasAtVisualMax)
	{
		...
		// go beyond its original monitor, so we adjust it again with mumo_PlaceRectInsideScreen.

		RECT rcBox0 = rectNow.StrechKeepCenter(
			RECTcx(pr->rectNewboxVisualMax), RECTcy(pr->rectNewboxVisualMax));
		RECT rcBox1 = mumo_PlaceRectInsideScreen(rcBox0, false, true);

		MoveWindow(hdlg, rcBox1.left, rcBox1.top, RECTcx(rcBox1), RECTcy(rcBox1), TRUE);
	}
...
fib_DlgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch(uMsg) 
	{{
		HANDLE_dlgMSG(hwnd, WM_INITDIALOG, fib_OnInitDialog);
		HANDLE_dlgMSG(hwnd, WM_DESTROY, fib_OnDestroy);
		HANDLE_dlgMSG(hwnd, WM_COMMAND, fib_OnCommand);
		HANDLE_dlgMSG(hwnd, WM_SIZE,          fib_OnSize); // JULayout
		HANDLE_dlgMSG(hwnd, WM_GETMINMAXINFO, fib_OnGetMinMaxInfo); // JULayout
		HANDLE_dlgMSG(hwnd, WM_TIMER, fib_OnTimer);
		HANDLE_dlgMSG(hwnd, WM_LBUTTONDOWN, fib_LButtonDown);
		HANDLE_dlgMSG(hwnd, WM_RBUTTONDOWN, fib_RButtonDown);
		HANDLE_dlgMSG(hwnd, WM_MOVE, fib_OnMove);
...
FIB_ret 
_ggt_FlexiInfobox(HWND hwndRealParent, const FibInput_st *p_usr_opt, const TCHAR *pszInfo)
{
/*
...
*/
	...
	*pword++ = 0x0000; // use system default dlgbox "window class"
	...
	wcscpy_s((wchar_t*)pword, ARRAYSIZE(szTitle), szTitle);
	...
	if(dt.style & DS_SETFONT)
	{
		...
		*pword++ = (WORD)_MAX_(6, fontsize); // minimum 6pt, but 6 will crop the icon above, why?
		...
		wcscpy_s((WCHAR*)pword, ARRAYSIZE(szFontface), szFontface);
	...
	*pword++ = 0xFFFF; *pword++ = 0x0080; // ctrl class is "button"
	wcscpy_s((wchar_t*)pword, 2, L"-"); pword+=2; // button text, change later in DlgProc
...
{
	return ns_fib::in_FlexiInfobox(hinstExeDll, resIdDlgbox, NULL, hwndRealParent, p_usr_opt, pszInfo);
}

FIB_ret 
ggt_FlexiInfobox(HWND hwndOwner, const FibInput_st *p_usr_opt, const TCHAR *pszInfo)
{
	return ns_fib::_ggt_FlexiInfobox(hwndOwner, p_usr_opt, pszInfo);
}
...
ggt_FlexiInfo(HWND hwndOwner, const TCHAR *pszInfo)
{
	FibInput_st si;
	si.szBtnOK = _T("OK");
	return ns_fib::_ggt_FlexiInfobox(hwndOwner, &si, pszInfo);
}
...
FIB_ret ggt_vlFlexiInfobox(
	HWND hwndOwner, const FibInput_st *p_usr_opt, const TCHAR *szfmt, va_list args)
{
	int reqlen_ = 1 + vsnTprintf(NULL, 0, szfmt, args);

	Cec_delete_pTCHAR bufInfo = new TCHAR[reqlen_];

	int req2 = vsnTprintf(bufInfo, reqlen_, szfmt, args);
...
FIB_ret 
ggt_vaFlexiInfobox(HWND hwndOwner, const FibInput_st *p_usr_opt, const TCHAR *szfmt, ...)
{
	va_list args;
	va_start(args, szfmt);
	FIB_ret fibret = ggt_vlFlexiInfobox(hwndOwner, p_usr_opt, szfmt, args);
	va_end(args);
	return fibret;
}

FIB_ret 
ggt_vaFlexiInfo(HWND hwndOwner, const TCHAR *szfmt, ...)
{
	va_list args;
	va_start(args, szfmt);
	...
	FibInput_st si;
	si.szBtnOK = _T("OK");
	FIB_ret fibret = ggt_vlFlexiInfobox(hwndOwner, &si, szfmt, args);
